  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/platform/health') {
        return json({ status: 'operational', version: '3.0.0', timestamp: new Date().toISOString() }, corsHeaders);
      }
      ...
      // Root redirect
      if (path === '/') {
        return Response.redirect(url.origin + '/platform', 302);
      }
